          ...
          <div className="grid grid-cols-1 gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {benefits.map((benefit, index) => (
                <div key={index} className="bg-gray-900 rounded-xl p-6 shadow-md border border-gray-800 hover:shadow-lg transition-all duration-300 hover:translate-y-[-5px]">
...
const VantaDotsGraph = () => {
  ...
  useEffect(() => {
    ...
    if (typeof window.VANTA !== 'undefined' && !isInitialized) {
      ...
      vantaEffect.current = window.VANTA.DOTS({
        ...
        scale: 1.00,
        scaleMobile: 1.00,
        color: 0x0ea2e9,  // Blue primary color
        color2: 0x6366f1, // Purple accent
  ...
  return (
    <div ref={vantaRef} className="w-full h-[400px] flex items-center justify-center">
      <div className="text-center z-10">
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
          Analytics Visualization
        </h2>
        <p className="text-xl text-gray-500 max-w-2xl mx-auto">
          Interactive data visualization to spot trends and opportunities
        </p>
      </div>
    </div>
  );
